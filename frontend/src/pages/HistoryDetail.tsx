...
const HistoryDetail: React.FC = () => {
  ...
  const { isGuestMode, getGuestConversation } = useGuestMode();
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const removeConversation = useMutation(api.conversations.remove);

  // Verify guest conversation exists
  const guestConv = isGuestMode && id ? getGuestConversation(id) : undefined;
  const notFound = isGuestMode && !guestConv;
      ...
      <div className="h-screen flex flex-col bg-gray-100">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <p className="text-gray-500 mb-4">This guest conversation no longer exists.</p>
            <button
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 overflow-hidden relative">
        ...
        <div className="max-w-5xl mx-auto h-full bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="bg-gray-50 px-6 py-3 border-b border-gray-200 flex justify-between items-center shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Guest mode: show temporary badge instead of delete button */}
              {isGuestMode ? (
                <div className="flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 px-3 py-1 rounded-full font-medium border border-amber-200">
                  <Clock size={12} />
                  Temporary
                </div>
              ) : (
      ...
      {/* Delete Confirmation Modal — not shown in guest mode */}
      {!isGuestMode && showDeleteModal && (
